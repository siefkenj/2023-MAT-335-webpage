...
%%%
\begin{document}
	\setheader{MAT335}{Homework 2}{Due: 11:59pm February 19}{}{}{}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item The \emph{unit $(n-1)$-simplex} is the set of all convex linear combinations
				of $\vec e_1,\ldots,\vec e_n$. Let $S$ be the unit $(n-1)$ simplex, and let $\mathcal P:\R^n\to\R^n$
		...
		\item We're going to prove some linear algebra facts because, \emph{just maybe} they'll be useful.
			...
			transformation induced by $P$ (i.e., given by matrix multiplication). Let $S$ be the unit $(n-1)$-simplex.
		\begin{enumerate}
	...
	\item {\bf Iterated Function Systems}
		\begin{enumerate}
			...
			\item Numpy can perform matrix multiplication with the
                                {\tt @} symbol. For example, the code
